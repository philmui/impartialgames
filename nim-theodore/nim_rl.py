...
class QAgent:
    ...
    #
    # Temporal Difference Update parameters:
    # ======================================
    # Gamma (γ): the discount rate. A value between 0 and 1. 
    #       The higher the value the less you are discounting.
    # Alpha (α): the learning rate. How much of the error should we accept 
    #       and therefore adjust our estimates towards. 
    #       A value between 0 and 1. A higher value adjusts aggressively, 
    #       accepting more of the error while a smaller one adjusts 
    #       conservatively but may make more conservative moves towards 
    #       the actual values.
    #
    def update_q_table(self, state, action, reward, next_state, flagged=False):
        ...
        self.q_table[str(state)][str(action)] = \
            (1 - self.learning_rate) * q_predict + \
            self.learning_rate * q_target
